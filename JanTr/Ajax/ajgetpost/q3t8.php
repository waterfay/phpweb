...
$movieOrder = array(//訂單
  array("id" => 1, "movieDay" => 1, "time" => 2, "sellout" => 2),//電影名稱 B 於時間 2021-01-26之第二場次 ,賣掉2張
...
switch ($_GET['do']) {
  ...
  case 'gettime':
    $result = array();
    foreach ($movieOrder as $row) 
    if ($row['movieDay'] == $_GET['mvid']) array_push($result, $row);
    echo json_encode($result, JSON_UNESCAPED_UNICODE);
    break;
}